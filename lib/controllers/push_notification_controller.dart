...
class PushNotificationController extends GetxController {
  ...
  var inform = RxList<NotificationModel>(); /** 메세지 온거 제목,내용 등 inform에 저장 */
  /// 안드로이드는 알림을 foreground, background 상관 없이 채널을 통해서 알림 보내야 한다
  late FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin; 
  late AndroidNotificationChannel channel;
  var onOff = true.obs; /// 처음에 true라고 가정
  ...
  void onInit() {
    ...
    initFirebaseMessaging();          /** 알림 내용 다루는 것 */
    getToken();
    super.onInit();
  }
  ...
  await FirebaseMessaging.instance.setForegroundNotificationPresentationOptions(
   ...
   NotificationSettings settings = await FirebaseMessaging.instance.requestPermission(
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) { /**앱 활성화된 상태(foreground) 콘솔에서 보낸 알림 message로 받아옴*/
    this.message.value = message; /**message값 업데이트 */
      RemoteNotification? notification = message.notification;


      if (notification != null && onOff.value) { /**서버로부터 알림이 온 상태이고, 알림 켜놨을 때  */
    ...
    //     print("-----------Debug----------------");
    //     print("-----------Debug----------------");
    //     for (NotificationModel notificationModel in inform) {
  ...
  //      print(' 알림 왔음 (Background 상태) :  ${message.data}');
...
static void showFlutterNotification(RemoteMessage message, FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin, AndroidNotificationChannel channel) {
  RemoteNotification? notification = message.notification;
  AndroidNotification? android = message.notification?.android;
  if (notification != null && android != null && !kIsWeb && PushNotificationController.to.onOff.value) { // 웹이 아니면서 안드로이드이고, push_notification의 onOff가 true인경우
       print(' 알림 왔음 (Background 상태) :  ${message.data}');
    flutterLocalNotificationsPlugin.show(
      notification.hashCode,
      notification.title,
